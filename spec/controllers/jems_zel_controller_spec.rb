require 'spec_helper'
# - - - - - - - - - - - - - - - - - - - - - -
# - Issue Ln114
# - - - - - - - - - - - - - - - - - - - - - -
# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe JemsController do

  # = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
  # - - - - - - - - - - - - - - - - - - - - - -
  # = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = 
  #  == Generated RSpecs suporting routes ==
  # - - - - - - - - - - - - - - - - - - - - - -
  # This should return the minimal set of attributes required to create a valid
  # Jem. As you add validations to Jem, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) { { "name" => "rails" } }

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # JemsController. Be sure to keep this updated too.
  let(:valid_session) { {} }

  describe "GET index" do
    it "assigns all jems as @jems" do
      jem = Jem.create! valid_attributes
      get :index, {}, valid_session
      assigns(:jems).should eq([jem])
    end
  end

  describe "GET show" do
    it "assigns the requested jem as @jem" do
      jem = Jem.create! valid_attributes
      get :show, {:name => jem.to_param}, valid_session
      assigns(:jem).should eq(jem)
    end
  end

  describe "GET new" do
    it "assigns a new jem as @jem" do
      get :new, {}, valid_session
      assigns(:jem).should be_a_new(Jem)
    end
  end

  describe "GET edit" do
    it "assigns the requested jem as @jem" do
      jem = Jem.create! valid_attributes
      get :edit, {:name => jem.to_param}, valid_session
      assigns(:jem).should eq(jem)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Jem" do
        expect {
          post :create, {:jem => valid_attributes}, valid_session
        }.to change(Jem, :count).by(1)
      end

      it "assigns a newly created jem as @jem" do
        post :create, {:jem => valid_attributes}, valid_session
        assigns(:jem).should be_a(Jem)
        assigns(:jem).should be_persisted
      end

      it "redirects to the created jem" do
        post :create, {:jem => valid_attributes}, valid_session
        response.should redirect_to(Jem.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved jem as @jem" do
        # Trigger the behavior that occurs when invalid params are submitted
        Jem.any_instance.stub(:save).and_return(false)
        post :create, {:jem => { "name" => "invalid value" }}, valid_session
        assigns(:jem).should be_a_new(Jem)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Jem.any_instance.stub(:save).and_return(false)
        post :create, {:jem => { "name" => "invalid value" }}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested jem" do
        jem = Jem.create! valid_attributes
        # Assuming there are no other jems in the database, this
        # specifies that the Jem created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Jem.any_instance.should_receive(:update).with({ "name" => "MyString" })
        put :update, {:name => jem.to_param, :jem => { "name" => "MyString" }}, valid_session
      end

      it "assigns the requested jem as @jem" do
        jem = Jem.create! valid_attributes
        put :update, {:name => jem.to_param, :jem => valid_attributes}, valid_session
        assigns(:jem).should eq(jem)
      end

      it "redirects to the jem" do
        jem = Jem.create! valid_attributes
        put :update, {:name => jem.to_param, :jem => valid_attributes}, valid_session
        response.should redirect_to(jem)
      end
    end

    describe "with invalid params" do
      it "assigns the jem as @jem" do

         #pending "Not sure what is broken"

       jem = Jem.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Jem.any_instance.stub(:save).and_return(false)
        put :update, {:name => jem.to_param, :jem => { "name" => "rails" }}, valid_session
      # put :update, {:name => jem.to_param, :jem => { "name" => "invalid value" }}, valid_session
        assigns(:jem).should eq(jem)
      end

      it "re-renders the 'edit' template" do
        jem = Jem.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Jem.any_instance.stub(:save).and_return(false)
        put :update, {:name => jem.to_param, :jem => { "name" => "invalid value" }}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested jem" do
      jem = Jem.create! valid_attributes
      expect {
        delete :destroy, {:name => jem.to_param}, valid_session
      }.to change(Jem, :count).by(-1)
    end

    it "redirects to the jems list" do
      jem = Jem.create! valid_attributes
      delete :destroy, {:name => jem.to_param}, valid_session
      response.should redirect_to(jems_url)
    end
  end

end
